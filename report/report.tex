...
\end{abstract}
	...
	The first order time derivative on the left hand side of equation \ref{eq:bare Schrodinger} is approximated using the Crank-Nicolson approach.
	This general approach is given by
	
	\begin{equation}
		\frac{u^{n+1}-u^{n}}{\Delta t} = \frac{1}{2}    \left(     F^{n+1} + F^n     \right),
	\end{equation}
	
	where $u^n$, in our case, is the dimentionless wavefunction in two dimentions. The superscript denotes the time step of $u$ and $\Delta t$ is the length of the time step, i.e. the spacing between points on the time axis.
	The $F^{n+1}$ and $F^n$ terms are given by the backward Euler and the forward Euler methods respectively. $F^n$ is therefore given by  
	\begin{equation}
		F^n = \frac{u^{n+1}-u^n}{\Delta t} 
	\end{equation}  	
	
	and 

	\begin{equation}
		F^{n} = \frac{u^{n}-u^{n-1}}{\Delta t}  \Rightarrow F^{n+1} = \frac{u^{n+1}-u^{n}}{\Delta t}
	\end{equation}  	
	


	We will solve this equation, in time, by discretizing it according to the Crank-Nicolson approach. 
	The double spatial derivatives will be discretized according to the Taylor approximations of second order. In the $x$ direction, the double spatial derivative is therefore given by
	\begin{equation}
		\frac{\partial^2 u}{\partial x^2} = \frac{u}{}
	\end{equation}
